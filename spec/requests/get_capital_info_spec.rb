...
RSpec.describe 'Get Capital Info' do
  ...
  it 'returns serialized error object JSON if invalid params' do
    country = 'Glorbiglorb'
    ...
    expect(parsed_response[:data]).to have_key(:id)
    expect(parsed_response[:data]).to have_key(:type)
    expect(parsed_response[:data][:type]).to eq('error')
    expect(parsed_response[:data]).to have_key(:attributes)
    expect(parsed_response[:data][:attributes]).to have_key(:code)
    expect(parsed_response[:data][:attributes]).to have_key(:message)
  end
end
